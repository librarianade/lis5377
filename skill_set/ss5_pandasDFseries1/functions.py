...
Reference (pandas boolean/count examples): https://note.nkmk.me/en/python-pandas-count-condition/
"""
...
def count_bool_cols(your_bool):
    ...
    print("\nDisplay type:")
    print(type(bool_sum_cols))  # Series
...
def count_bool_rows(your_bool):
    ...
    print("\nDisplay type:")
    print(type(bool_sum_rows))  # Series


def count_bool_total(your_bool):
    """function to count Boolean total"""
    bool_sum_total = your_bool.sum().sum()
    print(
        "\nPrint Boolean total:\n"
        "Note: Calling sum() on Series returns total count "
        "(also, implicitly converts to numpy.int64)."
    )
    print(bool_sum_total)

    print("\nDisplay type:")
    print(type(bool_sum_total))  # <class 'numpy.int64'>
...
def evaluate_series_data(s_data):
    ...
    print(
        "Note: sum() used on Series object implicitly converts to numpy.int64 "
        "(i.e., scalar value/single number)"
    )
...
def convert_dataframe_to_numpy_array(your_bool):
    ...
    print("\nPrint total number of states with two-letter abbreviation \"DE\":")
