...
def add_emails():
    """Accepts 0 args. Prompts for emails, returns user-entered values, including email count.
...
def get_phones(emails_list):
    """Accepts 1 arg. Prompts for phone numbers, based upon emails, returns user-entered values.
    With data validation!"""
    ...
    for my_iterator in range(len(emails_list)):
        while True:
            try:
                # prompt for phone numbers (Note: no newline, and counter variable to make "user-friendly.")
                print("{0} {1}".format(emails_list[i], ", phone:"))
                # or...
...
def create_contact(keys, values):
    ...
    # Note: zip() function joins elements in two lists; dict() converts resulting tuples into dictionary key-value pairs
